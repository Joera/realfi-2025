...
export async function createEnvironment(userConfig, resolvedExecutionParams) {
    ...
    const { deployments, migrations } = loadDeployments(deploymentsFolder, environmentName, false, context.fork
        ? undefined
        : {
            chainId,
            genesisHash,
