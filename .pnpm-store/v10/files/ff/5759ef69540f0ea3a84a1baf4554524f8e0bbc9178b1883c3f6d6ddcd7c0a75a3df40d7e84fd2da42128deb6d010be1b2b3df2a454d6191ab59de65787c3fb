...
describe('TransactionHashTrackerProvider', () => {
    ...
    describe('request method', () => {
        it('should forward non-transaction requests to the underlying provider', async () => {
            ...
            mockProvider.request.mockResolvedValue(mockResult);
    ...
    describe('transaction hash tracking', () => {
        ...
        it('should not track non-transaction requests', async () => {
            mockProvider.request.mockResolvedValue('0x123456');
    ...
    describe('edge cases', () => {
        ...
        it('should handle provider errors', async () => {
            ...
            await expect(tracker.request({ method: 'eth_sendTransaction', params: [{ from: '0x123' }] })).rejects.toThrow('Provider error');
            expect(tracker.transactionHashes).toHaveLength(0);
        });
        it('should handle provider rejections for non-transaction methods', async () => {
            const error = new Error('Network error');
            mockProvider.request.mockRejectedValue(error);
            await expect(tracker.request({ method: 'eth_getBalance', params: ['0x123', 'latest'] })).rejects.toThrow('Network error');
            expect(tracker.transactionHashes).toHaveLength(0);
        });
        ...
        it('should handle invalid transaction hash format', async () => {
            ...
            mockProvider.request.mockResolvedValue(invalidHash);
            const result = await tracker.request({
                method: 'eth_sendRawTransaction',
                params: ['0x123'],
            });
    ...
    describe('integration scenarios', () => {
        ...
        it('should handle batch of transactions', async () => {
            const txHashes = Array.from({ length: 5 }, (_, i) => `0x${i.toString().repeat(64)}`);
            for (const hash of txHashes) {
                mockProvider.request.mockResolvedValueOnce(hash);
            }
            for (let i = 0; i < 5; i++) {
...
//# sourceMappingURL=TransactionHashTracker.test.js.map