...
export async function getChainIdForEnvironment(config, environmentName, executionParams) {
    ...
    if (chainId && chainIdFromProvider && chainIdFromProvider != chainId) {
        console.warn(`provider give a different chainId (${chainIdFromProvider}) than the one expected for environment named "${environmentName}" (${chainId})`);
    }
    const chainIdToReturn = chainIdFromProvider || chainId;
    if (chainIdToReturn === undefined) {
        throw new Error(`Could not find chainId for environment named "${environmentName}" ${provider ? `` : '(no provider)'}`);
    }
    return chainIdToReturn;
}
export async function getChainIdForExecutionParams(config, executionParams) {
...
export function resolveExecutionParams(config, executionParameters, chainId) {
    ...
    const provider = 'provider' in actualChainConfig
        ? actualChainConfig.provider
        : new JSONRPCHTTPProvider(actualChainConfig.rpcUrl);
...
export async function loadEnvironment(config, executionParams, deploymentStore) {
    const userConfig = resolveConfig(config, executionParams.config);
...
Do you want to proceed (note that gas price can change for each tx)`,
        ...
        if (resolvedExecutionParams.reportGasUse) {
            ...
            const averageGasPrice = prices.length > 0 ? prices.reduce((sum, price) => sum + price, 0n) / BigInt(prices.length) : 0n;
