...
export function enhanceEnvIfNeeded(env, extensions) {
    ...
    for (const key in extensions) {
        if (!Object.prototype.hasOwnProperty.call(env, key)) {
            ...
            singleExtension[key] = extensions[key];
            const curriedFunction = withEnvironment(env, singleExtension);
            enhancedEnv[key] = curriedFunction[key];
        }
    }
    return enhancedEnv;
}
