...
export function mergeABIs(list, options) {
    const added = new Map();
    const mergedABI = [];
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        for (const element of listElem.abi) {
            if (element.type === 'function') {
                ...
                const selector = FunctionFragment.from(element).selector;
...
export function mergeArtifacts(list, options) {
    const { mergedABI, added, sigJSMap } = mergeABIs(list.map((v) => ({ name: v.name, abi: v.artifact.abi })), options);
