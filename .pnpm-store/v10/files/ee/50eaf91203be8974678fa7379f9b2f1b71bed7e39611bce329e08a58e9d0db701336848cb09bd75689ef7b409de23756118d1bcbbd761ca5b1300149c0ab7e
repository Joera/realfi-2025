...
function getChainsById(id) {
    const chains = chainById['' + id];
    return chains;
}
export function getDefaultChainInfoByName(name) {
...
// }
export function getDefaultChainInfoFromChainId(id) {
    let defaultChainInfo;
    const defaultChainInfos = getChainsById(id);
    if (defaultChainInfos && defaultChainInfos.length > 1) {
        console.warn(`chainId ${id} refers to different chain, please specific it by name or provide the chainConfig yourself`);
    }
    else {
        defaultChainInfo = defaultChainInfos ? defaultChainInfos[0] : undefined;
    }
...
export function getChainConfigFromUserConfigAndDefaultChainInfo(config, id, details) {
    ...
    if (canonicalName) {
        if (config.chains?.[id] && config.chains?.[canonicalName]) {
            throw new Error(`chain should be configured by chainId or name but not both, remove either ${id} or ${canonicalName}`);
        }
    }
    let chainConfig = canonicalName ? config.chains?.[canonicalName] : undefined;
    if (!chainConfig) {
    ...
    let deterministicDeployment = {
        ...
        create3: chainConfig.deterministicDeployment &&
